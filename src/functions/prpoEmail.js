...
function card2(doc,bk,x){ const a=x.c?(x.sum/x.c):0; return '<td width="192" valign="top" style="padding:0 5px;"><div style="background:'+GRAD[bk]+';background-image:linear-gradient(158deg,#ffffff 0%,'+GRAD[bk]+' 100%);border:1px solid #e8ecf2;border-top:3px solid '+COLOR[bk]+';border-radius:12px;padding:13px 14px;box-shadow:0 1px 3px rgba(15,23,42,.05);"><div style="min-height:26px;line-height:1.2;">'+chip(doc)+'<span style="font-family:'+FONT+';font-size:10px;font-weight:800;letter-spacing:.3px;color:#5b6b7f;text-transform:uppercase;">'+esc(bk)+'</span></div><div style="margin:6px 0 3px;white-space:nowrap;"><span style="font-family:'+FONT+';font-size:28px;font-weight:800;color:'+COLOR[bk]+';">'+x.n+'</span><span style="font-family:'+FONT+';font-size:13px;font-weight:800;color:'+RED+';"> ('+a.toFixed(1)+'d)</span></div><div style="font-family:'+FONT+';font-size:11.5px;font-weight:700;color:'+TEAL+';">AED '+money(x.amt)+'</div></div></td>'; }
...
function finding(L,col,title,right,narr,tbl){ const head='<div style="margin:0 0 6px;">'+badge(L,col)+'<span style="font-family:'+FONT+';font-weight:800;font-size:14px;color:'+NAVY+';vertical-align:middle;">'+title+'</span>'+(right?'<span style="float:right;font-family:'+FONT+';font-size:11px;font-weight:700;color:#94a3b8;">'+right+'</span>':'')+'</div>'; const body='<div style="font-family:'+FONT+';font-size:12.5px;color:#3f4b5b;line-height:1.5;margin:0 0 '+(tbl?'8px':'2px')+';">'+narr+'</div>'+(tbl||''); return '<div style="border:1px solid #e6ebf1;border-left:4px solid '+col+';border-radius:10px;background:#fff;padding:13px 15px;margin:0 0 12px;box-shadow:0 1px 3px rgba(15,23,42,.04);">'+head+body+'</div>'; }
